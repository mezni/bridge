...
# Save bearers to TinyDB with custom database name
def save_bearers_to_tinydb(bearers: List[Bearer], db_name: str):
    db = TinyDB(db_name)  # Open database by name
    table = db.table('bearers')  # Use 'bearers' table
    
    for bearer in bearers:
        # Insert each bearer as a new document in the table
        table.insert(bearer.to_dict())
    
    print(f"Saved {len(bearers)} bearers to TinyDB database '{db_name}'.")

# Example usage
num_bearers = 5  # Number of bearers to generate
generated_bearers = generate_bearers(num_bearers)

# Database name passed as a parameter
db_name = 'settings.json'  # You can specify any name here

# Save to TinyDB
save_bearers_to_tinydb(generated_bearers, db_name)
